...
public class VariantsOfTypesCombo<V extends IMetadata<V>> {
    ...
    public void registerVariants(IProxy proxy, ObjectType<V, ?, ?> type) {
        ...
        for (int subsetID : subsets) {
            ...
            String unlocName = getUnlocalizedPrefix() + type.getTranslationKey();

            if (block != null) {
                proxy.registerBlock(block, item, registryName, false);
                block.setTranslationKey(unlocName);
            ...
            item.setTranslationKey(unlocName);
     ...
     *                This will usually be gotten through <code>super.getTranslationKey(stack)</code>.
     * @return The unlocalized name for the stack.<br>
     */
    public String getTranslationKey(V variant, String base) {
        String variantName = variant.getTranslationKey();
     ...
     */
    public String getTranslationKey(ItemStack stack, String base) {
        V variant = getVariant(stack);

        if (variant == null)
            return getInvalidMetadataName();

        return getTranslationKey(variant, base);
    }
